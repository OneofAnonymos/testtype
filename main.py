...
async def test(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"تست شخصیت MBTI شروع شد!\nبرای پاسخ به هر سؤال، روی پیام ریپلای بزن و یکی از گزینه‌ها رو دقیق بنویس."
    )
...
async def send_question(update, context, user_id):
    ...
    reply_markup = ReplyKeyboardMarkup([[v] for k, v in options.items()], one_time_keyboard=True, resize_keyboard=True)
...
async def handle_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not last_msg_id or update.message.reply_to_message.message_id != last_msg_id:
        return
    ...
    if not matched:
        await update.message.reply_text("❗️ لطفاً یکی از گزینه‌های مشخص‌شده رو انتخاب کن.")
        return
...
async def compare(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("❌ شناسه عددی نامعتبر است.")
        return
